...
export class Storage extends StorageBase {
  ...
  /** Plain string in SecureStore — used for PIN (no JSON wrapping). */
  async secureGetRaw(key: string, fallback = ""): Promise<string> {
    try {
      const raw = await SecureStore.getItemAsync(key);
      ...
      this.warn("secureGetRaw", key, e);
      return fallback;
    }
  }

  async secureSetRaw(key: string, value: string): Promise<boolean> {
    try {
      await SecureStore.setItemAsync(key, value);
      return true;
    } catch (e) {
      this.warn("secureSetRaw", key, e);
      return false;
    }
  }
}
